...
parser.add_argument('-pp','--polymorphism_parameter',help='Parameters for calculating SSR polymorphism',required=True,type=float,default=0.7)
parser.add_argument('-index','--individual_variety_file',help='files that contains the relationship between individual(base_name) and variety',required=True,default='get_variety_from_filename')
...
if 1 in opertional_stage:
    ...
    if spm_type == 'pair':
        ...
        for file_prefix in all_individual:
            ...
            command ='rm '+file_prefix+'.unitigs.fa'
            print(command)
            os.system(command)
            ...
            if os.path.abspath('.') != input_path:
                print('not in the same path',os.path.abspath('.'),input_path)
